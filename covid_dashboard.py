...
st.set_page_config(
    page_title="COVID-19 Multi-Country Time Series Visualizer (2020-2023)",
    page_icon="🦠",
    layout="wide",
...
def load_data():
    ...
    # Use a fixed date from 2023 for the latest daily report (since JHU dataset was discontinued in March 2023)
    latest_data = pd.read_csv(
        'https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/03-09-2023.csv')
...
# Date range selector (added for 2020-2023 filter)
start_date = st.sidebar.date_input(
    "Start Date",
    datetime.date(2020, 1, 22)  # JHU dataset starts on 2020-01-22
)
end_date = st.sidebar.date_input(
    "End Date",
    datetime.date(2023, 3, 9)  # JHU dataset ends on 2023-03-10
)
...
def preprocess_data():
    ...
    # Filter for date range 2020-2023
    global_df = global_df[(global_df['Date'] >= pd.to_datetime(start_date)) & 
...
st.title("🦠 COVID-19 Multi-Country Time Series Visualizer (2020-2023)")
st.markdown(f"""
This dashboard visualizes COVID-19 data across multiple countries from {start_date} to {end_date}, including confirmed cases, 
...
if display_mode == 'Global Overview':
    ...
    # Global trend charts
    st.subheader("Global COVID-19 Trends (2020-2023)")
    ...
    with tab1:
        ...
        fig = px.line(global_df, x='Date', y=['Confirmed', 'Deaths', 'Recovered'],
                     title='Global Cumulative COVID-19 Cases (2020-2023)',
    ...
    with tab3:
        ...
        fig = px.line(global_df, x='Date', y=['Mortality_Rate', 'Recovery_Rate'],
                     title='Global COVID-19 Mortality and Recovery Rates (2020-2023)',
    ...
    with tab4:
        ...
        st.write("COVID-19 Geographic Distribution (Latest Data from 2023)")
        ...
        fig = px.scatter_geo(map_data, 
                            ...
                            title='Global COVID-19 Case Distribution (March 2023)',
    ...
    # Dual Y-Axis Plot
    st.subheader("Dual Y-Axis Plot: Active vs Total Cases (2020-2023)")
    ...
    fig.update_layout(
        title_text="Global COVID-19: Total vs Active Cases (2020-2023)",
        xaxis_title="Date",
    )
...
# ------------------- Step 7: Country Comparison -------------------
    ...
    st.header("Country Comparison (2020-2023)")
    ...
    if not selected_countries:
        ...
        with tab1:
            st.subheader("Confirmed Cases Comparison (2020-2023)")
            ...
            fig.update_layout(title="Confirmed COVID-19 Cases by Country (2020-2023)",
                             xaxis_title="Date",
            ...
            fig.update_layout(title=f"Daily New COVID-19 Cases ({window_size}-Day Moving Average) (2020-2023)",
                             xaxis_title="Date",
        ...
        with tab2:
            st.subheader("Deaths Comparison (2020-2023)")
            ...
            fig.update_layout(title="COVID-19 Deaths by Country (2020-2023)",
                             xaxis_title="Date",
                             yaxis_title="Deaths",
                             height=600)
            st.plotly_chart(fig, use_container_width=True)
            ...
            st.subheader("Mortality Rate Comparison (2020-2023)")
            fig = go.Figure()
            for country in selected_countries:
                df = country_data[country]
                fig.add_trace(go.Scatter(x=df['Date'], y=df['Mortality_Rate'], mode='lines', name=country))
            
            fig.update_layout(title="COVID-19 Mortality Rate by Country (2020-2023)",
                             xaxis_title="Date",
        ...
        with tab3:
            st.subheader("Active Cases Comparison (2020-2023)")
            ...
            fig.update_layout(title="Active COVID-19 Cases by Country (2020-2023)",
                             xaxis_title="Date",
        ...
        with tab4:
            st.subheader("Comparative Metrics (As of End Date)")
            
            # Create a dataframe for comparison
            latest_data = {}
            for country in selected_countries:
                df = country_data[country]
                if not df.empty:  # Check if dataframe is not empty after filtering
                    latest = df.iloc[-1]
                    latest_data[country] = {
            ...
            for metric in metrics:
                fig = px.bar(comparison_df, x='Country', y=metric, 
                             title=f"{metric} Cases by Country (as of {end_date})",
                             color='Country')
                st.plotly_chart(fig, use_container_width=True)
            ...
            for country in selected_countries:
                if country in comparison_df['Country'].values:  # Check if country exists in comparison data
...
# ------------------- Step 8: Prediction Models -------------------
    ...
    st.header("COVID-19 Prediction Models (2020-2023)")
    ...
    if country_df.empty:
        st.warning(f"No data available for {country_for_prediction} in the selected date range.")
    else:
        ...
        fig.update_layout(
            title=f"{model_type} Forecast for {country_for_prediction} (Based on 2020-2023 Data)",
            xaxis_title="Date",
            yaxis_title="Confirmed Cases",
            height=600
        )
...
*The JHU CSSE COVID-19 dataset was discontinued on March 10, 2023.*
